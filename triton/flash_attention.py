...
def _attn_fwd_inner(
    ...
    if STAGE == 1:
        ...
        # Used only for the block in there is transition between non-masked and masked keys
    ...
    for start_kv in range(lo, hi, BLOCK_SIZE_KV):
        ...
        if STAGE == 2:
            ...
            # compute the maximum value of qk or keep the old max
            m_ij = tl.maximum(m_i, tl.max(QK_block, 1) * softmax_scale)
            QK_block = QK_block * softmax_scale - m_ij[:, None]
        
        # compute the exponential of each dot, so now we are computing exp(qk_ij - m_ij)
        P_block = tl.math.exp(QK_block)
        # computing the sum by rows of the attention scores
        l_ij = tl.sum(P_block, 1)

        # this is the correction factor for the previous l_i
        alpha = tl.math.exp(m_i, m_ij)
        # Apply the corection factor to the previous l_i and add the new l_ij
        l_i = l_i * alpha + l_ij
...
def _attn_fwd(
    ...
    # this indicates which head and batch to process. Each program is 
    # associated with a signgle head of a batch
    ...
    # This allows to get the (N_CTX, HEAD_DIM) block i the Q, K, V by selecting indexing it by batch
    qvk_offset = (
...
def _attn_bwd_preprocess(
    ...
    # Load a single block of BLOCK_SIZE_Q rows of 0
    O_block = tl.load(
        O,
...
def _attn_bwd_dq(
    ...
    # because we need to use the range of offsets t apply the masking
    Q += offset_batch_head
    K += offset_batch_head
    V += offset_batch_head
    ...
    # Make sure the pointera are the right place w.r.t batch, head and sequence
    ...
    dO_block = tl.load(
        dO + offs_q[:, None] * stride_seq + offs_dim[None, :] * stride
    )
    ...
    for blk_idx in range(num_steps):
        ...
        if STAGE == 3:
            ...
            mask_block = offs_q[:, None] >= offs_kv[None, 0]
        ...
        # Compute dQ.
        # NOTE: We need to descale dq in the end, because kT was pre-scaled.
...
def _attn_bwd_dk_dv(
    ...
    # This is the offset that allows to select the right sequence given the batch and head.
    offset_batch_head_seq = (index_batch_head * SEQ_LEN).to(tl.int64)
    ...
    for blk_idx in range(num_steps):
        ...
        if STAGE == 3:
            ...
            # In this case we do not need to mask with -Inf before applying
            # the softmax since we already the normalization factors (stored in "m")
            P_T_block = tl.where(mask_block, P_T_block, P_T_block, 0.0)
        
        dO_block = tl.load(dO_ptrs)
        # According to the formula: dV_new = dV_old + P^T x dO, where x is the matrix multiplication
        dV_block += tl.dot(P_T_block.to(float16), dO_block)
